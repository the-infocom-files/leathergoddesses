...
enchantment.\"" CR>)
	       (<VERB? EXAMINE>
		<COND (<NOUN-USED ,W?ENGRAV ,BLENDER>
...
Earth, and dickory wood from the jungles of Venus. A path leads south.")
	...
	(NORTH "If you want to jump in the canal, say so.")
	(NE "If you want to jump in the canal, say so.")
	(NW "If you want to jump in the canal, say so.")
	(FLAGS RLANDBIT ONBIT)
	(GLOBAL CANAL-OBJECT DOCK-OBJECT WATER)>

<OBJECT MOUSE
	(IN HICKORY-AND-DICKORY-DOCK)
	(DESC "mouse")
...
"The barge is unsinkable. (Then again, so was the Titanic.)" CR>)
		...
		<TELL ,PERIOD-CR>)
	       (<VERB? EXAMINE>
		<COND (<NOUN-USED ,W?CONTROL ,BARGE>
		       <TELL
...
<ROOM CANAL
      ...
      (NORTH "If you want to jump in the canal, say so.")
      (NE "If you want to jump in the canal, say so.")
      (EAST "If you want to jump in the canal, say so.")
      (SE "If you want to jump in the canal, say so.")
      (SOUTH "If you want to jump in the canal, say so.")
      (SW "If you want to jump in the canal, say so.")
      (WEST "If you want to jump in the canal, say so.")
      (NW "If you want to jump in the canal, say so.")
      (FLAGS ONBIT)
      (GLOBAL CANAL-OBJECT DOCK-OBJECT WATER DUNES SIGN)
      (ACTION CANAL-F)
...
<ROUTINE I-ION-DEATH ()
	 ...
	 <COND (<G? ,ION-DEATH-COUNTER 4>
		<JIGS-UP
"Your anatomy, in absorbing a dose of super-ionized energy in trans-lethal
...
<ROUTINE MEANT-OTHER-HOLE () ;"if portable and non-portable hole both present"
	 <COND (<OR <AND <PRSO? ,FLEXIBLE-HOLE>
			 <EQUAL? <GET ,P-ADJW 0> ,A?FLEXIBLE ,A?PORTABLE>>
		    <AND <PRSI? ,FLEXIBLE-HOLE>
			 <EQUAL? <GET ,P-ADJW 1> ,A?FLEXIBLE ,A?PORTABLE>>>
		<RFALSE>)
...
the end of the dock, to the east.")
      ...
      (GLOBAL CANAL-OBJECT DOCK-OBJECT WATER STAIRS)
...
<ROUTINE WELL-F ()
	 <COND (<VERB? LOOK-INSIDE REACH-IN>
		...
		<RTRUE>)
	       (<AND <VERB? PUT>
		     <PRSI? ,PSEUDO-OBJECT> ;"that is, the well"
...
<ROUTINE PICK-WIFE ("OPTIONAL" (OBJ <>) "AUX" DUPE)
	 <COND (<OR <EQUAL? .OBJ ,INTNUM>
		...
		<COND (<L? ,P-NUMBER 1>
		      ...
		      (<SET DUPE <ZMEMQ ,P-NUMBER ,WRONG-ANSWERS 7>>
		       <TELL
...
;<ROOM WESTERN-EDGE-OF-DESERT
      (IN ROOMS)
      (DESC "Western Edge of Desert")
      (WEST TO RUINED-CASTLE-2)
      (EAST PER DESERT-MAZE-ENTER-F)
      (FLAGS RLANDBIT ONBIT)
      (ACTION EDGE-OF-DESERT-F)>

;<ROOM EASTERN-EDGE-OF-DESERT
      (IN ROOMS)
      (DESC "Eastern Edge of Desert")
      (WEST PER DESERT-MAZE-ENTER-F)
      (EAST TO OASIS)
      (FLAGS RLANDBIT ONBIT)
      (ACTION EDGE-OF-DESERT-F)>

;<ROUTINE EDGE-OF-DESERT-F (RARG)
	 <COND (<EQUAL? .RARG ,M-LOOK>
		<TELL "You'd be advised to follow the path to the ">
		<COND (<EQUAL? ,HERE ,WESTERN-EDGE-OF-DESERT>
		       <TELL "we">)
		      (T
		       <TELL "ea">)>
		<TELL "st, for to the ">
		<COND (<EQUAL? ,HERE ,WESTERN-EDGE-OF-DESERT>
		      ...
		      (T
		       <TELL "we">)>
		<TELL
"st lies tractless, uncharted desert of mind-numbing dimension.">)
	       (<AND <EQUAL? .RARG ,M-END>
		     ,ENTERED-MAZE-FROM
		     <NOT <FSET? ,LOST-IN-DESERT ,MUNGBIT>>>
		<FSET ,LOST-IN-DESERT ,MUNGBIT>
		<SETG ENTERED-MAZE-FROM <>>
		<SETG AWAITING-REPLY 2>
		<QUEUE I-REPLY 2>
		<TELL "   [It wasn't a very good maze, was it?]" CR>)>>

;<GLOBAL ENTERED-MAZE-FROM <>>

;<ROUTINE DESERT-MAZE-ENTER-F ()
	 <SETG ENTERED-MAZE-FROM ,HERE>
	 ,LOST-IN-DESERT>

;<ROOM LOST-IN-DESERT
      (IN ROOMS)
      (DESC "Lost in the Desert")
      (LDESC
"You are hopelessly lost in a maze of twisty paths winding among towering
sand dunes.")
      (NORTH PER LOST-IN-DESERT-EXIT-F)
      (NE PER LOST-IN-DESERT-EXIT-F)
      (EAST PER LOST-IN-DESERT-EXIT-F)
      (SE PER LOST-IN-DESERT-EXIT-F)
      (SOUTH PER LOST-IN-DESERT-EXIT-F)
      (SW PER LOST-IN-DESERT-EXIT-F)
      (WEST PER LOST-IN-DESERT-EXIT-F)
      (NW PER LOST-IN-DESERT-EXIT-F)
      (FLAGS RLANDBIT ONBIT)
      (GLOBAL DUNES)>

;<ROUTINE LOST-IN-DESERT-EXIT-F ()
	 <COND (<EQUAL? ,ENTERED-MAZE-FROM ,WESTERN-EDGE-OF-DESERT>
		,EASTERN-EDGE-OF-DESERT)
	       (T
		,WESTERN-EDGE-OF-DESERT)>>

<ROOM WATTZ-UPP-DOCK
      (IN ROOMS)
      (DESC "Wattz-Upp Dock")
...
south polar cap.")
      ...
      (WEST TO OASIS)
      (FLAGS RLANDBIT ONBIT)
      (GLOBAL CANAL-OBJECT DOCK-OBJECT WATER SIGN)
      (THINGS <PSEUDO (RED BUOY BUOY-F)
		      (WARNING BUOY BUOY-F)>)>
...
read them again, follow the faint trails to the north or east.")
      (NORTH TO TUNDRA)
      (NE "You'd only get lost in the snow and die.")
      (EAST TO PENGUIN-PARK)
      (SE "You'd only get lost in the snow and die.")
      (SOUTH "You'd only get lost in the snow and die.")
      (SW "You'd only get lost in the snow and die.")
      (WEST "You'd only get lost in the snow and die.")
      (NW "You'd only get lost in the snow and die.")
      (FLAGS RLANDBIT ONBIT)
      (GLOBAL HOLE)
...
north.")
      (NORTH TO GYPSY-CAMP)
      (SOUTH "This is as far south as you can go!")
      (EAST "You walk in a tight circle, returning to your starting point.")
      (WEST "You walk in a tight circle, returning to your starting point.")
      (IN TO ORPHANAGE-FOYER IF ORPHANAGE-DOOR IS OPEN)
      (FLAGS RLANDBIT ONBIT)
      (GLOBAL SIGN IGLOO ORPHANAGE-DOOR WINDOW)
      (ACTION SOUTH-POLE-F)
...
<ROOM ORPHANAGE-FOYER
      ...
      (FLAGS RLANDBIT ONBIT INDOORSBIT)
      (GLOBAL ORPHANAGE-DOOR IGLOO WINDOW)
